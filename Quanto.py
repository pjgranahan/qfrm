...
class Quanto(OptionValuation):
    ...
    def calc_px(self, method='BS', nsteps=None, npaths=None, keep_hist=False, vol_ex=0.0, correlation=0.0, seed=1,
        ...
        >>> s = Stock(S0=1200, vol=.25, q=0.015)
        >>> o = Quanto(ref=s, right='call', K=1200, T=1, rf_r=.03, frf_r=0.05)
        >>> print(o.calc_px(method='MC', nsteps=10, npaths=5,vol_ex=0.12, correlation=0.2).px_spec.px)
        162.72494855715664
    ...
    def _calc_MC(self):
        ...
        S = _.ref.S0 * np.exp(
            np.cumsum(
                rnd.normal(
                    (_.frf_r - foreign_numeraire_dividend_yield - 0.5 * _.ref.vol ** 2) * dt,
                                 _.ref.vol * np.sqrt(dt), (n_steps + 1, n_paths)
                ),
                axis=0
            )
        )
        S[0] = _.ref.S0
        payout = np.maximum(signCP * (S - _.K), 0)
