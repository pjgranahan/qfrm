...
class VarianceSwap(OptionValuation):
    ...
    def calc_px(self, method='BS', rf_r=0.03, T=1., K=(280.,300.,320.,340.,360.,380.,400.), vol=(0.2,0.2,0.2,0.3,0.3,0.3,0.3), 
        ...
        T : float 
                Required: Time to maturity of the variance swap
        K : float
        ...
        >>> #Pricing by BSM
        >>> s = Stock(355)
        >>> o = VarianceSwap(ref=s)
        >>> o.calc_px().px_spec.px
        -489162.761418
        >>> ##Changing the stock price
        >>> VarianceSwap(ref=Stock(310)).calc_px().px_spec.px
        -504216.712984
        >>> VarianceSwap(ref=Stock(500)).calc_px().px_spec.px
        -1404368.57684
        >>> ##Explicit input parameters
        >>> VarianceSwap(ref=Stock(290)).calc_px(method='BS', rf_r=0.03, T=1., K=(280.,300.,320.,340.,360.,380.,400.), \
        ...
        >>> VarianceSwap(ref=Stock(S0=1020,q=.01)).calc_px(method='BS', rf_r=.04, T=.25, K=Karr, \
        ...
        >>> plt.show()
        >>> px
        (210.0654998613123, 209.86542915602709, 209.64712658791012, ... -181.33277290957744, -185.12452059271672, -188.93450013868775)
         

        """
        
        #should override OptionValuation K; Stock vol with vector values
        self.rf_r, self.T, self.K, self.ref.vol, self.L_Var, self.Var_K = rf_r, T, K, vol, L_Var, Var_K
...
##Changing the stock price
print(VarianceSwap(ref=Stock(310)).calc_px().px_spec.px)
print(VarianceSwap(ref=Stock(500)).calc_px().px_spec.px)
##Explicit input parameters
print(VarianceSwap(ref=Stock(290)).calc_px(method='BS', rf_r=0.03, T=1., K=(280.,300.,320.,340.,360.,380.,400.), \
                vol=(0.2,0.2,0.2,0.3,0.3,0.3,0.3), L_Var=10000000., Var_K=0.1).px_spec.px)
##Example 26.4 on Hull p 614
Karr = (800,850,900,950,1000,1050,1100,1150,1200)
varr = (.29,.28,.27,.26,.25,.24,.23,.22,.21)
print(VarianceSwap(ref=Stock(S0=1020,q=.01)).calc_px(method='BS', rf_r=.04, T=.25, K=Karr, \
                vol=varr, L_Var=100., Var_K=.045).px_spec.px)
##Price vs. the strike volatility curve - example of vectorization of price calculation
import matplotlib.pyplot as plt
from numpy import linspace
s2K = linspace(0.01,0.20,200)
o = VarianceSwap(ref=Stock(355))
px = tuple(map(lambda i: o.calc_px(Var_K=s2K[i]**2).px_spec.px/1000, range(s2K.shape[0])))
fig = plt.figure()
ax = fig.add_subplot(111)
ax.plot(s2K,px,label='Variance Swap')
